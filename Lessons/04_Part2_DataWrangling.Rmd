...
# Format sampledate as date using lubridate
...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #Drop level removes other columns from dataset.
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments)
#double check the object class of sampledate column. It's a factor, not date
...
#Wrangling using pipe formating
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #specify the data set
  mutate(month = month(sampledate)) %>% #creating a new month column from the month info in sampledate.
  select(lakeid:daynum, month, sampledate:comments) %>% #reordering columns to have new month columns after daynum 
  drop_na(depth) #remove rows with NAs in depth column.
...
# write a more succinct line of code to subset the nutrient dataset. Using ':' for adjacent columns instead of typing out each col name
...
We will work with the new functions from `tidyr`. For gather we will use `pivot_longer` and for spread we will use `pivot_wider`. Note that if you are familiar with `gather` and `spread` will may still use it, they are not going away, but they are not under active development. Gather=transforming columns into rows. Spreads turns rows into columns. 
...
# Gather nutrient data into one column using pivot_longer. Specify columns to be transformed to rows, and then two new column names
...
#use functions 'subset' and '!is.na'  to remove concentrations that are NA or don't have a value
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))

#Count occurrence by nutrient
...
# Split components of cells into multiple columns using seperate function 
...
'rbind': combine data frames when all column names match
...
```{r joining data}
#combines all rows and columns from both the phys and nutrient datasets 
...
### rbind
rbind combines datasets by row
...
#left join keeps the # of rows from Litter, and adds the unique columns from Trap
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID"))

dim(Litter)
dim(Trap)
dim(LitterTrap)
#select columns of interest in LitterTrap dataset using pipe notation
LitterTrap <- LitterTrap %>%
  select(plotID:trapID, collectDate, functionalGroup:qaDryMass, subplotID:geodeticDatum)
#save tidy data set as processed data file
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>% #filter for surface data
  group_by(lakename, month) %>% #group by lakename and month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #remove rows with NAs in these 3 columns
