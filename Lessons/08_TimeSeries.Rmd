...
library(tidyverse)
library(lubridate)
install.packages("trend") # to run stationary test
library(trend) 
install.packages("zoo") # to replace misisng observations in time series
library(zoo)
...
For time series analysis, need observations to be evenly spaced through time (daily, monthly, yearly, etc.)
Time series are a special class of dataset, where a response variable is tracked (indexed) over time. The frequency of measurement and the timespan of the dataset can vary widely. 

At its most simple, a time series model includes an explanatory time component and a response variable. 

Mixed models can include additional explanatory variables (check out the `nlme` and `lme4` R packages). We will cover a few simple applications of time series analysis in these lessons, with references for how to take analyses further.

Amount of data needed depends on how far out you are trying to forecast, but generally the more the better.
...
* **Data gaps**: Data are not collected at regular intervals, necessitating *interpolation* between measurements. Long gaps between data points may require splitting up data sets.
...
```{r}
#First step in TSA is to plot the data 
...
#create data set for observations before 1985
EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01"))

#create data set for observations after 1985
...
2. A **trend** component, which quantifies the upward or downward progression over time. The trend component of a time series does not have to be monotonic, i.e. stochastic(changing through time). 

3. An **error** or **random** component, which makes up the remainder of the time series after other components have been accounted for. This component reflects the noise in the dataset. Part of the time series that can't be explained by the trend of seasonality. 
...
```{r}
#creating timeseries object, selecting the variable of interest (discharge.mean), define starting point as Jan 1985, and the frequency (in this case daily)
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365)
```

STL FUNCTION (vector with time stamp)

The `stl` function decomposes the time series object into its component parts.

important arguments:
x= UNIVARIATE time series to be decomposed\
s.window 

We must specify that the window for seasonal extraction is either "periodic" or a specific number of at least 7. The decomposition proceeds through a loess (locally estimated scatterplot smoothing) function.
...
# Visualize the decomposed series. Creates four plots showing the: observed data, seasonal component, trend component, random component.
...
mutate(EnoDischarge.late_Components,
...
Two types of trends may be present in our time series dataset: **monotonic/deterministic** or **stochastic**. 
Monotonic trends are a gradual shift over time that is consistent in direction (either increasing or decreasing), for example in response to land use change. 
...
In general, detecting a monotonic trend requires a long sequence of data with few gaps. 

If we are working with monthly data, a time series of at least five years is recommended. 

Gaps can be accounted for, but a gap that makes up more than 1/3 of the sampling period is generally considered the threshold for considering a gap to be too long (a step trend analysis might be better in this situation). 
...
Zoo (package) 'na.approx()' function 

* **Spline**: similar to a linear interpolation except that a quadratic function is used to interpolate rather than drawing a straight line. 'na.spline()' function 
...
For time series analysis, if NAs are in middle of series, never omit them, must replace with approximate values using interpolation 

Interpolation methods will not change the min, max, mean, etc. values for dataset.


```{r}
head(wind_data)
summary(wind_data$AWND)
# if working with packages that have functions with same name, specify by package name :: function
# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
# na.approx() function cleans dataset, interpolate missing observations by linear method
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall), or function 'smk.test() (package: trend)

* **Spearman Rho**: no seasonality, non-parametric, missing data allowed. Function: `cor.test(method="spearman")` (package: stats)

to remove seasonal component, can run tests on trend and random component. 
...
ADF used to detect a stochastic trend.

If you have a stochastic trend, before fitting a time series model to data set, must differentiate the series.
...
# Generate time series (trend test needs ts, not data.frame). Using function first() to define f_month as first month from $DATE column
...
#plotting decomposition shows that there is seasonal component to data.

# Run SMK test (Seasonal Mann-Kendal Test, only test which can handle seasonality)
...
#p-value <0.05 meaning that we reject the null hypothesis that the series is stationary, meaning there is a trend in the series (alternative hypothesis)
...
> Answer: Visualizing the linear regression can see that the slope is negative, or there is a decreasing trend in the average wind speed over time. 
