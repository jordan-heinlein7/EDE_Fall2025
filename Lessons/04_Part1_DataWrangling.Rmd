...
> Add notes here: plant productivity, nutrient cycling in lake systems. 
...
# Format the sampledate column in NTL.phys.data as date using lubridate and mdy format.
...
`==`equals 
`!='values different, or to exclude a catergorical variable
`<`less than
`<=`less than or equal to
`>`greater than
`>=`greater than or equal to
`&`use for multiple conditions, both x and y must be true
`|`use for multiple conditions, either x or y. Can also just use a ','comma
...
#use filter function and '|' to select data for only Paul Lake and Peter Lake:
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
#use filter function and != to filter out data based on the following lake names:
...
#Alternative to filter for Paul and Peter lake using neater code. Operator '%in%' and vector of names.
...
# ':' used to select adjacent columns
```
### Mutate
...
```{r mutating data}
#for numeric data, can use mutate to create new data columns by manipulating data. Ex converting temp in C to F.
...
```{r using lubridate}
# add a month column to the dataset using mutate() and month() from lubridate.
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) 

# reorder columns to put month with the rest of the date variables using select()
...
#use function first, last if the column is not arranged. This is better approach.
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% #specify the dataset. In a pipe, this is only done once.
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #1st filter command, then
  select(lakename, sampledate:temperature_C) %>% #second command, then
  mutate(temperature_F = (temperature_C*9/5) + 32) # third command
#when writing a pipe, end each line of code with '%>%' 
```
...
```{r exporting data}
#When creating a processed dataset, save or write the file to a csv using write.csv()
