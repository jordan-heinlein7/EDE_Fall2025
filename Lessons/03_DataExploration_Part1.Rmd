...
#Use getwd() to reveal what R sees as the working directory. This is where R is looking for files. 
getwd()
...
# Relative file path (friendly for users regardless of machine). Saves content as an object (USGS.flow,data) data as a dataframe in environment
USGS.flow.data <- read.csv(                                   
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE)   
#Important to always include the stringsAsFactors = TRUE when reading a data set.
...
#If we don't assign a name to the imported data set, the data populates in the console only, and does not create a dataframe in the coding environment.
...
> ANSWER: The README file associated with the USGS dataset is saved in the Data>Metadata folder. Metadata is where you can find out more information on the dataset, parameters, how it was produced, etc. Having a seperate metadata file that explains the set allows for reproducivle analysis because it ensures that everyone working with the data set understands it (reliability, what the observations and attributes are, etc.)
...
# Rename columns by creating a vector with desired column names. Vector length must be equal to the number of columns in the data table.
...
str(USGS.flow.data) #shows structure including the number of observations (rows) and variables (columns), and the data type each column was imported as Ex. date information should be date/time not factor or numerical, etc.
dim(USGS.flow.data) #gives dimensions of data (observations, attributes)
length(USGS.flow.data) # of columns or attributes

head(USGS.flow.data) #shows top portion of dataset
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #specific sub sample of dataframe, displaying rows 30,000-30005, and rows 3,8,14.

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #summary command gives summary stats (min,max, median, mean, quartiles) for all numeric columns. 

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: R will assign them NA if numeric data column, or leave blank for other data types. 
...
```{r Finding NAs}
summary(USGS.flow.data$discharge.mean) #will show the number of NAs 
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #na.omit removes any rows that have missing data. 
dim(USGS.flow.data)
dim(USGS.flow.data.complete)
#can't calculate the mean with mean command if there are missing values
...
```{r}
USGS.flow.data$site_no <- as.factor(USGS.flow.data$site_no) #use as.factor
str(USGS.flow.data)
```
...
USGS.flow.data2  <- read.csv(
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE,
  colClasses= c('site_no'='factor') #This is telling R to import the site_no data as factors not integers.
  ) 
```
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #as.Date converts the date column to date/time data type. dates formatted with / in raw table. Elements of date in raw dataframe may be seperated by - or spaces. 
```
...
```{r Fixing dates}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$data$datetime, "%m/%d/%y")
...
write.csv(
  ...
  row.names=FALSE)
#now that data has been manipulated, save in Processed folder. When you save data, the dates are stored as a string, so they will need to be corrected again. 
```
