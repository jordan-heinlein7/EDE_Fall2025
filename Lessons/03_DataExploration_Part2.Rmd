...
>Answer: To ensure that all of the information is stored in a way that is usable or will not interfere with our analyses (ex dates are dates). 
...
library(tidyverse)
library(here)
# 2. Check the working directory is EDE_Fall2025
getwd()
here()
# 3. Import datasets
USGS.flow.data <- read.csv(
  file=here('Data/Processed/USGS_Site02085000_Flow_Processed.csv'), stringsAsFactors = TRUE, 
...
#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), bins = 50) + #color set to other categorical variable in dataset
  scale_x_continuous(limits = c(0, 10)) +
